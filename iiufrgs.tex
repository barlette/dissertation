...
\title{Variability and Voltage Scaling Aware FinFET Design}
...
{
Projetos em FinFET robustos à escala da tensão de alimentação e nível de variabilidade de processo
...
%\cite{nassif:08} \cite{bernstein2006high} \cite{young1994failure} \cite{posser2016electromigration} \cite{takeda1983empirical} \cite{nassif:08} \cite{bernstein2006high} \cite{stine1997analysis} \cite{dietrich2011process} \cite{asenov2003simulation} \cite{frank2001device,wong1999nanoscale}
...
\end{itemize}
    ...
    \begin{figure} []
        \centering
        \includegraphics[width=0.9\textwidth, trim={0.5cm 6cm 6cm 4cm},clip]{transistorVariability.pdf}
...
The simulations will be carried out in HSPICE \footnote{https://www.synopsys.com/} using the netlist obtained after the physical verification flow and the parasitic capacitances extraction. The deviation on the device geometry impacts the electrical parameter WF causing high fluctuations \cite{zimpeck2016}. It happens due to the orientation of metal grains randomly aligned in FinFET manufacturing process. In this way, WFF represents the most significant variation beyond the other parameters \cite{meinhardt2014impact}. The process variability evaluation will be taken through 2000 Monte Carlo (MC) simulations \cite{alioto2015variations} varying the WF of devices according to a Gaussian distribution considering a 3\(\sigma\) deviation. The reference values from ASAP7 technology for electrical simulations are shown in Table \ref{electPar}.
...
Given so, all simulations were automatized through shell script. For each scenario the 2000 MC simulations would take place, and the script would detect how much errors there was by counting the number of error messages, detected through regular expression rules. If the number of error messages were higher than 10\% (200 simulations), the current frequency of operation would be decreased by a predetermined and arbritary margin based on the maximum operating frequency of similar scenarios.

For example, since the traditional inverter with 5 fins at 1\% WFF and 0.7V of supply voltage, worked at about 25.3GHz, it is safe to assume a decrease step of a 100MHz, while at scenaios with lower supply voltages and higher variability levels, the decrease step could go down to the tens of KHz. If the number of errors is below 10\% the frequency will be increased by the same margin. A version of the shell script is shown in Annex E. Additionally, all measures were automatically parsed and arranged in tables, through shell scripts as well, for easier handling and to make the process of generating results less time consuming.
...
\begin{table}[H]
\centering
\caption{Noise Margins for each design as supply voltage scales.}
\label{tab:SNM}
...
%since due to the variability scaling and its related increase on off-current, the ratios will tend to decrease. This result is due to the leakage current suppression embedded into the ST design through the feed-back system. Therefore,
...
Given all the concepts, results and discussion, the contributions of this work surround: the understanding of the effects of transistor sizing and different levels of supply voltage and process variability. The analysis of different inverter designs in search of a replacement of the traditional CMOS inverter for low energy consumption and high robustness. It was shown that the traditional inverter still is the circuit with best performance, energy efficiency, and smaller area. The 6T ST presents better results concerning the signal quality, robustness to noise, and overall current deviations. The TIST, although being a specific case, has shown high gains, hysteresis, and even noise margins, specially at very low supply voltage of 100mV. The SIG did present improvements over some metrics deviations, although at signal quality and noise margins, it showed a very similar behavior to the traditional inverter.

Given so, the replacement of traditional inverters with the considered inverter designs is a question of application and priorities. If the priorities are performance, area utilization, and energy consumption at higher level  of supply voltage, the traditional inverter still is an appropriate circuit. If overall robustness, variability and noise-wise is a priority, the 6T ST should be considered. It can not yet be affirmed, since the simulations were not performed, but the TIST presented acceptable results at the lowest considered supply voltage (100mV) which could show a hint of the capabilities of the TIST to work at ultra low supply voltages.

The impact of the replacement of traditional inverters are evaluated on different Full Adders designs aiming to achieve a decrease into the impact of the process variability into the circuits metrics. Overall, it was shown frequency and energy, decrease and increase, respectively due to process variability. It was shown the increase in the currents with the leakage currents presenting a much higher scaling factor in comparison to the dynamic current, resulting in lower current ratios. Further, the leakage currents normalized deviations did not present considerable improvements over supply voltage increase. These results turn the leakage current relationship with process variability an evermore important challenge to be addressed. The replacement of traditional inverter in the Full Adders demonstrated the viability of this technique to reduce the effects of process variability in circuits, especially those based on pass-transistor logic.
...
\chapter{Shell Script responsible for the Spice Simulations Automation}
\begin{lstlisting}
#!/bin/bash

outputs="ST_1F ST_2F ST_3F ST_4F ST_5F"
number=0
freq_text=0
new_freq_text=0
freq=0
...
path="${HOME}/Desktop/script/ST/1WFF/1"

for output in $outputs
do
if [ $done -eq 1 -a $new_freq -ne 0 ]
then
freq_text=$(cat "$path/${output}.sp" | grep -m 1 "Hz" | awk '{print $1}')
echo $freq_text
time=$(cat "$path/${output}.sp" | grep -m 1 "freq1" | awk '{print $4}')
echo $time
sed -i 's/'$freq_text'/'$new_freq_text'/g' "$path/${output}.sp"
sed -i 's/'$time'/'$new_time'/g' "$path/${output}.sp"
fi
rm $path/${output}
hspice "$path/${output}.sp" >> $path/${output}
number=$(grep -c "failed" $path/${output})
echo $number
done=0
...
time=$(cat "$path/${output}.sp" | grep -m 1 "freq1" | awk '{print $4}')
echo $time
freq=$(echo "${freq_text:1:${#freq_text}-3}")
echo $freq
new_freq=$(expr $freq - 50000) #10khz steps
if [ $new_freq -ne 0 ]
then
echo $new_freq
new_time=$(echo "scale=4; 500000000 / $new_freq" | bc)
echo $new_time
new_freq_text="*${new_freq}Hz"
new_time="${new_time}ns"
echo $new_freq_text
sed -i 's/'$freq_text'/'$new_freq_text'/g' "$path/${output}.sp"
sed -i 's/'$time'/'$new_time'/g' "$path/${output}.sp"
rm $path/${output}
...
if [ $number -gt 200 ]
then
sed -i 's/'$new_freq_text'/'$freq_text'/g' "$path/${output}.sp"
sed -i 's/'$new_time'/'$time'/g' "$path/${output}.sp"
done=1
fi
fi
done
echo "done with $path/${output}"
done
\end{lstlisting}
